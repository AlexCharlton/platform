 ...
 */
    ...
    case DDR_TRAINING_INIT:
        ...
        retry_count = 0U;
#ifdef DEBUG_DDR_INIT
        (void)uprint32(g_debug_uart, "\n\r Start training. TIP_CFG_PARAMS:", LIBERO_SETTING_TIP_CFG_PARAMS);
#endif
...
/* To verify if separate reset required for DDR4 - believe it is not */
    ...
    case DDR_TRAINING_ROTATE_CLK:
        ...
        CFG_DDR_SGMII_PHY->expert_dlycnt_mv_rd_dly_reg.expert_dlycnt_mv_rd_dly_reg = 0x1FU;
        ...
        CFG_DDR_SGMII_PHY->expert_dlycnt_mv_rd_dly_reg.expert_dlycnt_mv_rd_dly_reg = 0x0U;
...
#ifndef RENODE_DEBUG
        if ((DDRCFG->DFI.STAT_DFI_INIT_COMPLETE.STAT_DFI_INIT_COMPLETE & 0x01U) == 0x01U)
#endif
        {
...
static uint8_t memory_tests(void)
{
    ...
    while (shift_walking_one <= 28U) /* 28 => 1G */
    {
        ...
        if (shift_walking_one >= 4U)
        {
            start_address = (uint64_t)((((0x1U << (shift_walking_one + 1)) - 1U) - 0x0F));
            error = mtc_sanity_check(start_address);
...
static void load_dq(uint8_t lane)
{
    ...
    else
    {
        CFG_DDR_SGMII_PHY->expert_dlycnt_move_reg1.expert_dlycnt_move_reg1 =
            (CFG_DDR_SGMII_PHY->expert_dlycnt_move_reg1.expert_dlycnt_move_reg1 & (uint32_t)~0x0FU);
    }
    /* set expert_dfi_status_override_to_shim = 0x7 */
    ...
    {
        CFG_DDR_SGMII_PHY->expert_dlycnt_load_reg1.expert_dlycnt_load_reg1 =
            (CFG_DDR_SGMII_PHY->expert_dlycnt_load_reg1.expert_dlycnt_load_reg1 & (uint32_t)~0x0FU);
    }
    /* set expert_mode_en = 0x8 */
    CFG_DDR_SGMII_PHY->expert_mode_en.expert_mode_en = 0x8U;
}
...
static void increment_dq(uint8_t lane, uint32_t move_count)
{
    ...
    if (lane < 4U)
    {
        CFG_DDR_SGMII_PHY->expert_dlycnt_direction_reg0.expert_dlycnt_direction_reg0 = (0xFFU << (lane * 8U));
    }
    else
    {
    ...
    while (move_count)
    {
        ...
        if (lane < 4U)
        {
            CFG_DDR_SGMII_PHY->expert_dlycnt_move_reg0.expert_dlycnt_move_reg0 = (0xFFU << (lane * 8U));
        }
        else
        {
            CFG_DDR_SGMII_PHY->expert_dlycnt_move_reg1.expert_dlycnt_move_reg1 |= 0x0FU;
...
write_calibration_using_mtc(uint8_t number_of_lanes_to_calibrate)
{
    ...
    for (cal_data = 0x00000U; cal_data < 0xfffffU; cal_data = cal_data + 0x11111U)
    {
             ...
             laneToTest++)
        {
            ...
            {
                if ((calib_data.write_cal.status_lower & (0x01U << laneToTest)) == 0U) /* Still looking for good value */
                {
