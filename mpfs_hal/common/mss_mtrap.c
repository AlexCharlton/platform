...
#define E51_LOCAL_NUM_SOURCES 48U
    ...
    void handle_m_ext_interrupt(void)
    {
        ...
        volatile uint32_t int_num = PLIC_ClaimIRQ_static();
        ...
        PLIC_CompleteIRQ_static(int_num);

        if (EXT_IRQ_DISABLE == disable)
        {
            PLIC_DisableIRQ_static((PLIC_IRQn_Type)int_num);
        }
    }
    ...
    void trap_from_machine_mode(uintptr_t *regs, uintptr_t dummy, uintptr_t mepc)
    {
        ...
        if (((mcause & MCAUSE_INT) == MCAUSE_INT) && ((mcause & MCAUSE_CAUSE) >= IRQ_M_LOCAL_MIN) && ((mcause & MCAUSE_CAUSE) <= IRQ_M_LOCAL_MAX))
        {
