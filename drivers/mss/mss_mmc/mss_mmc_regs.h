...
#define SRS12_ERROR_CMD_LINE (SRS12_COMMAND_TIMEOUT_ERROR | SRS12_COMMAND_CRC_ERROR | SRS12_COMMAND_END_BIT_ERROR | SRS12_COMMAND_INDEX_ERROR)
...
#define GetByte(dword, byte_nr) (((dword) >> ((byte_nr) * 8u)) & 0xFFu)
    ...
    static inline uint8_t SDCARD_SWITCH_FUNC_IS_FUNC_SUPPORTED(const uint8_t *val, uint8_t groupNum, uint8_t funcNum)
    {
        ...
        if (funcNum < 32u)
        {
            ...
            supportStatus = (((uint32_t)GET_BYTE_FROM_BUFFER2(val, 64u, 50u - offset) | (uint32_t)(GET_BYTE_FROM_BUFFER2(val, 64u, 51u - offset))) << 8) & (uint32_t)(1u << funcNum);
        }
    ...
    static inline uint8_t SDCARD_SWICH_FUNC_GET_BUSY_STAT(const uint8_t *val, uint8_t groupNum, uint8_t funcNum)
    {
        ...
        if (funcNum < 32u)
        {
            ...
            busyStatus = ((GET_BYTE_FROM_BUFFER2(val, 64u, 34u - offset) | GET_BYTE_FROM_BUFFER2(val, 64u, 35u - offset)) << 8) & (uint32_t)(1u << funcNum);
        }
...
/* All errors mask */
#define SDCARD_RESP_R1_ALL_ERRORS (SDCARD_RESP_R1_ILLEGAL_CMD_ERR | SDCARD_RESP_R1_COM_CRC_ERR | SDCARD_RESP_R1_ERASE_SEQUENCE_ERR | SDCARD_RESP_R1_ADDRESS_ERR | SDCARD_RESP_R1_PARAM_ERR)
