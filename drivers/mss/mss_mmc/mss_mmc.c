...
#define NULL_POINTER ((void *)0u)
        ...
        const mss_mmc_cfg_t *cfg)
    {
        ...
        PLIC_DisableIRQ_static(MMC_main_PLIC);
                    ...
                    if (TRANSFER_IF_SUCCESS == response_status)
                    {
                        ...
                        if (MSS_MMC_TRANSFER_SUCCESS == ret_status)
                        {
                            ...
                            case MSS_MMC_MODE_HS400_ES:
                                ...
                                if (((hw_device_type & DEVICE_SUPPORT_HS400) == DEVICE_SUPPORT_HS400) && (hw_strobe_suport == MMC_SET))
                                {
        ...
        /* Clear interrupts */
        MMC->SRS12 = ~(SRS12_CURRENT_LIMIT_ERROR | SRS12_CARD_INTERRUPT | SRS12_CARD_REMOVAL | SRS12_CARD_INSERTION);
        return (ret_status);
    }
    ...
    *******************************************************************************/
        ...
        uint32_t *dst_addr)
    {
        ...
        if (g_mmc_init_complete == MMC_SET)
        {
            ...
            if (TRANSFER_IF_SUCCESS == response_status)
            {
                ...
                srs03_data = (uint32_t)(SRS3_DATA_PRESENT | SRS3_TRANS_DIRECT_READ | SRS3_BLOCK_COUNT_ENABLE | SRS3_RESP_ERR_CHECK_EN | SRS3_RESP_INTER_DISABLE | SRS3_RESPONSE_CHECK_TYPE_R1 | SRS3_RESP_LENGTH_48 | SRS3_CRC_CHECK_EN | SRS3_INDEX_CHECK_EN);
        ...
        uint32_t size)
    {
        ...
        if (g_mmc_init_complete == MMC_SET)
        {
            ...
            else
            {
                ...
                if (((size % blocklen) != MMC_CLEAR) || (size > (SIZE_32MB - BLK_SIZE)) || (size == MMC_CLEAR) || (dest == NULL_POINTER))
                {
                ...
                else
                {
                    ...
                    if (TRANSFER_IF_SUCCESS == response_status)
                    {
                        ...
                        PLIC_EnableIRQ_static(MMC_main_PLIC);
                        ...
                        {
                            /* DPS, Data transfer direction - read */
                            srs03_data = ((uint32_t)(SRS3_DATA_PRESENT | SRS3_TRANS_DIRECT_READ | SRS3_MULTI_BLOCK_SEL | SRS3_BLOCK_COUNT_ENABLE | SRS3_RESPONSE_CHECK_TYPE_R1 | SRS3_RESP_LENGTH_48 | SRS3_CRC_CHECK_EN | SRS3_INDEX_CHECK_EN | SRS3_DMA_ENABLE));
                        ...
                        {
                            /* DPS, Data transfer direction - read */
                            srs03_data = ((uint32_t)(SRS3_DATA_PRESENT | SRS3_TRANS_DIRECT_READ | SRS3_BLOCK_COUNT_ENABLE | SRS3_RESPONSE_CHECK_TYPE_R1 | SRS3_RESP_LENGTH_48 | SRS3_CRC_CHECK_EN | SRS3_INDEX_CHECK_EN | SRS3_DMA_ENABLE));
        ...
        uint16_t data_size)
    {
        ...
        if (g_mmc_init_complete == MMC_SET)
        {
            if ((data_size != MMC_CLEAR) && (data_size <= BLK_SIZE) && (dst_addr != NULL_POINTER))
            {
                ...
                /* DPS, Data transfer direction - read */
                srs03_data = (uint32_t)(SRS3_DATA_PRESENT | SRS3_TRANS_DIRECT_READ | SRS3_BLOCK_COUNT_ENABLE | SRS3_RESP_ERR_CHECK_EN | SRS3_RESP_INTER_DISABLE | SRS3_RESPONSE_CHECK_TYPE_R5 | SRS3_RESP_LENGTH_48 | SRS3_CRC_CHECK_EN | SRS3_INDEX_CHECK_EN);
                /* Check cmd and data line busy */
                do
                {
                    srs9 = MMC->SRS09;
    ...
    *******************************************************************************/
        ...
        uint32_t dst_addr)
    {
        ...
        if (g_mmc_init_complete == MMC_SET)
        {
            ...
            if (TRANSFER_IF_SUCCESS == response_status)
            {
                ...
                MMC->SRS01 = (BLK_SIZE | (MMC_SET << BLOCK_COUNT_ENABLE_SHIFT));
                /* DPS, Data transfer direction - read */
                srs03_data = (uint32_t)(SRS3_DATA_PRESENT | SRS3_TRANS_DIRECT_WRITE | SRS3_BLOCK_COUNT_ENABLE | SRS3_RESP_ERR_CHECK_EN | SRS3_RESP_INTER_DISABLE | SRS3_RESPONSE_CHECK_TYPE_R1 | SRS3_RESP_LENGTH_48 | SRS3_CRC_CHECK_EN | SRS3_INDEX_CHECK_EN);
                /* Check cmd and data line busy */
                do
                {
                    srs9 = MMC->SRS09;
        ...
        uint32_t size)
    {
        ...
        if (g_mmc_init_complete == MMC_SET)
        {
            ...
            {
                /* Size should be divided by 512, not greater than (32MB - 512) */
                if (((size % blocklen) != MMC_CLEAR) || (size > (SIZE_32MB - BLK_SIZE)) || (size == MMC_CLEAR) || (src == NULL_POINTER))
                {
                    ret_status = MSS_MMC_INVALID_PARAMETER;
                }
                else
                {
                    ...
                    if (TRANSFER_IF_SUCCESS == response_status)
                    {
                        ...
                        if (blockcount > MMC_SET)
                        {
                            ...
                            srs03_data = (uint32_t)(SRS3_DATA_PRESENT | SRS3_TRANS_DIRECT_WRITE | SRS3_MULTI_BLOCK_SEL | SRS3_BLOCK_COUNT_ENABLE | SRS3_RESPONSE_CHECK_TYPE_R1 | SRS3_RESP_LENGTH_48 | SRS3_CRC_CHECK_EN | SRS3_INDEX_CHECK_EN | SRS3_DMA_ENABLE);
                            /* Multi block transfer */
                            g_mmc_is_multi_blk = MMC_SET;
                        ...
                        {
                            /* DPS, Data transfer direction - write */
                            srs03_data = (uint32_t)(SRS3_DATA_PRESENT | SRS3_TRANS_DIRECT_WRITE | SRS3_BLOCK_COUNT_ENABLE | SRS3_RESPONSE_CHECK_TYPE_R1 | SRS3_RESP_LENGTH_48 | SRS3_CRC_CHECK_EN | SRS3_INDEX_CHECK_EN | SRS3_DMA_ENABLE);
                            /* Single block transfer */
                            g_mmc_is_multi_blk = MMC_CLEAR;
                            /* Execute command */
                            MMC->SRS02 = argument;
                ...
                else
                {
                    ...
                    if (TRANSFER_IF_SUCCESS == response_status)
                    {
                        ...
                        if (ret_status != MSS_MMC_INVALID_PARAMETER)
                        {
                            ...
                            MMC->SRS01 = (blocklen | (blockcount << BLOCK_COUNT_ENABLE_SHIFT));
                            /* enable interrupts */
                            MMC->SRS14 = (SRS14_COMMAND_COMPLETE_SIG_EN | SRS14_TRANSFER_COMPLETE_SIG_EN | SRS14_DATA_TIMEOUT_ERR_SIG_EN | SRS14_ADMA_ERROR_SIG_EN);
                            PLIC_EnableIRQ_static(MMC_main_PLIC);
                            /* Check data line busy */
                            do
                            {
        ...
        uint16_t data_size)
    {
        ...
        if (g_mmc_init_complete == MMC_SET)
        {
            if ((data_size != MMC_CLEAR) && (data_size <= BLK_SIZE) && (src_addr != NULL_POINTER))
            {
                ...
                /* DPS, Data transfer direction - read */
                srs03_data = (uint32_t)(SRS3_DATA_PRESENT | SRS3_TRANS_DIRECT_WRITE | SRS3_BLOCK_COUNT_ENABLE | SRS3_RESP_ERR_CHECK_EN | SRS3_RESP_INTER_DISABLE | SRS3_RESPONSE_CHECK_TYPE_R5 | SRS3_RESP_LENGTH_48 | SRS3_CRC_CHECK_EN | SRS3_INDEX_CHECK_EN);
                /* Check cmd and data line busy */
                do
                {
                    srs9 = MMC->SRS09;
                ...
                arguement = SDIO_WRITE | (SDIO_CSA_REG_ADDR << SHIFT_9BIT) | SDIO_OPCODE_INC | ((data_size == (uint16_t)BLK_SIZE) ? MMC_CLEAR : (uint32_t)data_size);
                /* Command argument */
                MMC->SRS02 = arguement;
                /* Execute command */
                MMC->SRS03 = (uint32_t)((SDIO_CMD_53_IO_RW_EXTENDED << MMC_SRS03_COMMAND_SHIFT) | srs03_data);
    ...
    mss_mmc_status_t MSS_MMC_error_recovery(void)
    {
        ...
        if (((trans_status_isr) & (SRS12_COMMAND_TIMEOUT_ERROR | SRS12_COMMAND_CRC_ERROR | SRS12_COMMAND_END_BIT_ERROR | SRS12_COMMAND_INDEX_ERROR | SRS12_RESPONSE_ERROR)) != MMC_CLEAR)
        {
            /* Read device status */
            response_status = cif_send_cmd(sdcard_RCA << SHIFT_16BIT,
                                           MMC_CMD_13_SEND_STATUS,
        ...
        else if (((trans_status_isr) & (SRS12_DATA_END_BIT_ERROR | SRS12_DATA_CRC_ERROR | SRS12_DATA_TIMEOUT_ERROR)) != MMC_CLEAR)
        {
        ...
        {
            mmc_delay(DELAY_COUNT);
            if ((MMC->SRS09 & (SRS9_DAT0_SIGNAL_LEVEL | SRS9_DAT1_SIGNAL_LEVEL | SRS9_DAT2_SIGNAL_LEVEL | SRS9_DAT3_SIGNAL_LEVEL)) == MMC_CLEAR)
            {
                response_status = TRANSFER_IF_FAIL;
            }
            else
                    ...
                    if (TRANSFER_IF_SUCCESS == response_status)
                    {
                        ...
                        if (MSS_MMC_TRANSFER_SUCCESS == ret_status)
                        {
                            ...
                            if (MMC_SET == hw_cq_mode_en)
                            {
                                ...
                                reg = (uint32_t)(CQRS05_TASK_CLEARED_STAT_EN | CQRS05_RESP_ERR_STAT_EN | CQRS05_TASK_COMPLETE_STAT_EN);
                                MMC->CQRS05 = reg;
        ...
        uint32_t size)
    {
        ...
        if (g_mmc_cq_init_complete == MMC_SET)
        {
            ...
            else
            {
                ...
                if (((size % blocklen) != MMC_CLEAR) || (size > (SIZE_1GB - (CQ_HOST_NUMBER_OF_TASKS * BLK_SIZE))) || (size == MMC_CLEAR) || (src == NULL_POINTER))
                {
                    ret_status = MSS_MMC_INVALID_PARAMETER;
                }
                else
                {
                    ...
                    if (TRANSFER_IF_SUCCESS == response_status)
                    {
                        ...
                        for (task_id = MMC_CLEAR; ((task_id < CQ_HOST_NUMBER_OF_TASKS) && (blockcount != MMC_CLEAR)); ++task_id)
                        {
                            ...
                            /* Supports only 64KB */
                            dcmdTaskDesc[BYTES_4] = (uint32_t)((CQ_DESC_VALID | CQ_DESC_ACT_TRAN | CQ_DESC_END) | (((size >= MASK_16BIT) ? MMC_CLEAR : size) << SHIFT_16BIT));
                        ...
                        /* Enable interrupts */
                        MMC->SRS14 = (SRS14_CMD_QUEUING_SIG_EN | SRS14_COMMAND_TIMEOUT_ERR_SIG_EN | SRS14_DATA_TIMEOUT_ERR_SIG_EN);
                        PLIC_EnableIRQ_static(MMC_main_PLIC);
        ...
        uint32_t size)
    {
        ...
        if (g_mmc_cq_init_complete == MMC_SET)
        {
            ...
            {
                /* Size should be divided by 512, not greater than (1GB - 16KB) */
                if (((size % blocklen) != MMC_CLEAR) || (size > (SIZE_1GB - (CQ_HOST_NUMBER_OF_TASKS * BLK_SIZE))) || (size == MMC_CLEAR) || (dest == NULL_POINTER))
                {
                    ret_status = MSS_MMC_INVALID_PARAMETER;
                }
                else
    ...
    *******************************************************************************/
        ...
        if (MSS_MMC_TRANSFER_IN_PROGRESS == g_mmc_trs_status.state)
        {
            if ((g_device_hpi_suport & DEVICE_HPI_SUPPORT) == DEVICE_HPI_SUPPORT)
            {
                ...
                if (TRANSFER_IF_SUCCESS == response_status)
                {
                    do
                    {
                        command_information = (uint32_t)(SRS3_ABORT_CMD | SRS3_RESPONSE_CHECK_TYPE_R1 | SRS3_RESP_LENGTH_48 | SRS3_CRC_CHECK_EN | SRS3_INDEX_CHECK_EN);
                ...
                else
                {
                    ...
                    if (blockcount > MMC_SET)
                    {
                        ...
                        response_status = cif_send_cmd(blockcount | (MMC_SET << SHIFT_30BIT),
                                                       ...
                                                       MSS_MMC_RESPONSE_R1);
                        /* Enable interrupts */
                        MMC->SRS14 = (SRS14_COMMAND_COMPLETE_SIG_EN | SRS14_TRANSFER_COMPLETE_SIG_EN | SRS14_DMA_INTERRUPT_SIG_EN | SRS14_DATA_TIMEOUT_ERR_SIG_EN);
                        PLIC_EnableIRQ_static(MMC_main_PLIC);
        ...
        uint32_t size)
    {
        ...
        if (g_mmc_init_complete == MMC_SET)
        {
            ...
            {
                /* Size should be divided by 512, not greater than (32MB - 512) */
                if (((size % blocklen) != MMC_CLEAR) || (size > (SIZE_32MB - BLK_SIZE)) || (size == MMC_CLEAR) || (dest == NULL_POINTER) || (packed_header == NULL_POINTER))
                {
                    ret_status = MSS_MMC_INVALID_PARAMETER;
                }
                else
    ...
    *******************************************************************************/
        ...
        uint32_t data_sz)
    {
        ...
        if (datasize >= SIZE_64KB)
        {
            ...
            while (size > MMC_CLEAR)
            {
                ...
                adma_descriptor_table[j] = (ADMA2_DESCRIPTOR_TYPE_TRAN | ADMA2_DESCRIPTOR_VAL | ADMA2_DESCRIPTOR_INT | ((current_subsize & MASK_16BIT) << SHIFT_16BIT));
                j++;
        ...
        {
            adma_descriptor_table[j] = (ADMA2_DESCRIPTOR_TYPE_TRAN | ADMA2_DESCRIPTOR_VAL | ADMA2_DESCRIPTOR_INT | ((datasize & MASK_16BIT) << SHIFT_16BIT));
            j++;
            adma_descriptor_table[j] = ((uint32_t)buf_address & MASK_32BIT);
            j++;
            adma_descriptor_table[j] = (uint32_t)((uint64_t)buf_address >> MMC_64BIT_UPPER_ADDR_SHIFT);
            ...
            if (TRANSFER_IF_SUCCESS == response_status)
            {
                ...
                if (TRANSFER_IF_SUCCESS == response_status)
                {
                    ...
                    if ((S18A != MMC_CLEAR) && (TRANSFER_IF_SUCCESS == response_status))
                    {
                        ...
                        const uint32_t dat_level = (SRS9_DAT0_SIGNAL_LEVEL | SRS9_DAT1_SIGNAL_LEVEL | SRS9_DAT2_SIGNAL_LEVEL | SRS9_DAT3_SIGNAL_LEVEL);
        ...
        uint8_t raw)
    {
        ...
        if (transfer_direction == MMC_CLEAR)
        {
            for (i = MMC_CLEAR; i < number_of_bytes; i++)
            {
                ...
                argument = (uint32_t)((register_address + i) << SHIFT_9BIT) | (raw << SHIFT_27BIT) | (uint32_t)(transfer_direction << SHIFT_31BIT);
        ...
        else
        {
            ...
            for (i = MMC_CLEAR; i < number_of_bytes; i++)
            {
                ...
                argument = (uint32_t)((register_address + i) << SHIFT_9BIT) | (raw << SHIFT_27BIT) | (uint32_t)(transfer_direction << SHIFT_31BIT) | *data_byte;
                data_byte++;
        ...
        uint8_t raw)
    {
        ...
        if (transfer_direction == MMC_CLEAR)
        {
            ...
            {
                /* Set address of register to read */
                argument = (uint32_t)((register_address_temp + i) << SHIFT_9BIT) | (raw << SHIFT_27BIT) | (uint32_t)(transfer_direction << SHIFT_31BIT);
        ...
        else
        {
            ...
            {
                /* Set address of register to write */
                argument = (uint32_t)((register_address_temp + i) << SHIFT_9BIT) | (raw << SHIFT_27BIT) | (uint32_t)(transfer_direction << SHIFT_31BIT) | *data_byte;
                data_byte++;
        ...
        uint8_t group_num)
    {
        ...
        {
            argumentBase = (uint32_t)(MASK_24BIT & ~(MASK_4BIT << shift)) | ((uint32_t)function_num << shift);
        }
    ...
    static mss_mmc_status_t sd_card_uhsi_supported(void)
    {
        ...
        if (shift < SHIFT_32BIT)
        {
            argument = (uint32_t)(MASK_24BIT & ~(MASK_4BIT << shift)) | ((uint32_t)function_num << shift) | (uint32_t)SDCARD_SWITCH_FUNC_MODE_CHECK;
        }
    ...
    static uint8_t is_uhsi_supported(void)
    {
        ...
        else
        {
            ...
            uint32_t uhsi_modes = (uint32_t)(SRS17_SDR50_SUPPORTED | SRS17_SDR104_SUPPORTED | SRS17_DDR50_SUPPORTED);
    ...
    static uint8_t set_host_sdclk(uint32_t frequencyKHz)
    {
        ...
        for (i = 1u; i < 2046u; i++)
        {
            if (((baseclkkHz / i) < frequencyKHz) || (((baseclkkHz / i) == frequencyKHz) && ((baseclkkHz % i) == 0u)))
            {
                break;
            }
        }
        ...
        temp |= (temp1 & 0xFF00u) | ((temp1 & 0x30000u) >> 10) | (uint32_t)SRS11_INT_CLOCK_ENABLE;
