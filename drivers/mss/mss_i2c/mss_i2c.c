...
#define MSS_I2C1_HI_BASE (I2C_TypeDef *)0x2810B000u
        ...
        uint8_t options)
    {
        ...
        PLIC_EnableIRQ_static(this_i2c->irqn);
        ...
        mss_i2c_instance_t *this_i2c)
    {
        ...
        case ST_TX_DATA_ACK:
            ...
            else /* Done sending. let's stop */
            {
                ...
                {
                    PLIC_DisableIRQ_static(this_i2c->irqn);
                    clear_irq = 0u;
                }
        ...
        uint8_t irq_type)
    {
        ...
        if ((this_i2c == &g_mss_i2c0_lo) || (this_i2c == &g_mss_i2c0_hi))
        {
            if (irq_type & MSS_I2C_SMBALERT_IRQ)
            {
                ...
                PLIC_EnableIRQ_static(I2C0_ALERT_PLIC);
            }
            if (irq_type & MSS_I2C_SMBSUS_IRQ)
            {
                this_i2c->hw_reg->SMBUS |= SMBSUS_IE_MASK;
                /* Enable the interrupt at the PLIC. */
                PLIC_EnableIRQ_static(I2C0_SUS_PLIC);
            }
        }
        else
        {
            if (irq_type & MSS_I2C_SMBALERT_IRQ)
            {
                this_i2c->hw_reg->SMBUS |= SMBALERT_IE_MASK;
                /* Enable the interrupt at the PLIC. */
                PLIC_EnableIRQ_static(I2C1_ALERT_PLIC);
            }
            if (irq_type & MSS_I2C_SMBSUS_IRQ)
            {
                this_i2c->hw_reg->SMBUS |= SMBSUS_IE_MASK;
                /* Enable the interrupt at the PLIC. */
                PLIC_EnableIRQ_static(I2C1_SUS_PLIC);
            }
        }
    }
        ...
        uint8_t irq_type)
    {
        ...
        if ((this_i2c == &g_mss_i2c0_lo) || (this_i2c == &g_mss_i2c0_hi))
        {
            if (irq_type & MSS_I2C_SMBALERT_IRQ)
            {
                ...
                this_i2c->hw_reg->SMBUS &= ~SMBALERT_IE_MASK;
                PLIC_DisableIRQ_static(I2C0_ALERT_PLIC);
            }
            if (irq_type & MSS_I2C_SMBSUS_IRQ)
            {
                /* Disable interrupt at the PLIC and the MSS I2C. */
                this_i2c->hw_reg->SMBUS &= ~SMBSUS_IE_MASK;
                PLIC_DisableIRQ_static(I2C0_SUS_PLIC);
            }
        }
        else
        {
            if (irq_type & MSS_I2C_SMBALERT_IRQ)
            {
                /* Disable interrupt at the PLIC and the MSS I2C. */
                this_i2c->hw_reg->SMBUS &= ~SMBALERT_IE_MASK;
                PLIC_DisableIRQ_static(I2C1_ALERT_PLIC);
            }
            if (irq_type & MSS_I2C_SMBSUS_IRQ)
            {
                /* Disable interrupt at the PLIC and the MSS I2C. */
                this_i2c->hw_reg->SMBUS &= ~SMBSUS_IE_MASK;
                PLIC_DisableIRQ_static(I2C1_SUS_PLIC);
            }
        }
    }
