    ...
    MSS_MAC_init(mss_mac_instance_t *this_mac, mss_mac_cfg_t *cfg)
    {
...
#endif /* defined(TARGET_G5_SOC) */
        {
...
#endif
            {
                 ...
                 */
                PLIC_DisableIRQ_static(this_mac->mac_q_int[0]);
                PLIC_SetPriority_static(this_mac->mac_q_int[0], cfg->queue0_int_priority);

#if defined(TARGET_G5_SOC)
                PLIC_DisableIRQ_static(this_mac->mac_q_int[1]);
                PLIC_SetPriority_static(this_mac->mac_q_int[1], cfg->queue1_int_priority);
                PLIC_DisableIRQ_static(this_mac->mac_q_int[2]);
                PLIC_SetPriority_static(this_mac->mac_q_int[2], cfg->queue2_int_priority);
                PLIC_DisableIRQ_static(this_mac->mac_q_int[3]);
                PLIC_SetPriority_static(this_mac->mac_q_int[3], cfg->queue3_int_priority);
                PLIC_DisableIRQ_static(this_mac->mmsl_int);
                PLIC_SetPriority_static(this_mac->mmsl_int, cfg->mmsl_int_priority);
#endif
            }
            mac_reset();
                        ...
                        mss_mac_rx_int_ctrl_t enable)
    {
        ...
        if (MSS_MAC_AVAILABLE == this_mac->mac_available)
        {
            if (0U == this_mac->queue[queue_no].in_isr)
            {
                ...
                {
                    PLIC_DisableIRQ_static(this_mac->mac_q_int[queue_no]); /* Single interrupt from GEM? */
                }
            }
...
#warning "Nothing implemented for multi packet tx yet"
                           ...
                           uint32_t tx_count)
    {
        ...
        PLIC_DisableIRQ_static(this_mac->mac_q_int[0]); /* Single interrupt from GEM? */
        ...
        *p_nw_control = *p_nw_control | GEM_TRANSMIT_START;
        ...
        PLIC_EnableIRQ_static(this_mac->mac_q_int[0]); /* Single interrupt from GEM? */
...
// #endif
    ...
    if (&g_mac0 == this_mac)
    {
        ...
        if (0U != this_mac->use_local_ints)
        {
            PLIC_DisableIRQ_static(MAC0_INT_PLIC);
            PLIC_DisableIRQ_static(MAC0_QUEUE1_PLIC);
            PLIC_DisableIRQ_static(MAC0_QUEUE2_PLIC);
            PLIC_DisableIRQ_static(MAC0_QUEUE3_PLIC);
            PLIC_DisableIRQ_static(MAC0_MMSL_PLIC);
    ...
    else if (&g_mac1 == this_mac)
    {
        ...
        if (0U != this_mac->use_local_ints)
        {
            PLIC_DisableIRQ_static(MAC1_INT_PLIC);
            PLIC_DisableIRQ_static(MAC1_QUEUE1_PLIC);
            PLIC_DisableIRQ_static(MAC1_QUEUE2_PLIC);
            PLIC_DisableIRQ_static(MAC1_QUEUE3_PLIC);
            PLIC_DisableIRQ_static(MAC1_MMSL_PLIC);
    ...
    else if (&g_emac0 == this_mac)
    {
        ...
        if (0U != this_mac->use_local_ints)
        {
            PLIC_DisableIRQ_static(MAC0_EMAC_PLIC);
    ...
    else
    {
        if (&g_emac1 == this_mac)
        {
            ...
            if (0U != this_mac->use_local_ints)
            {
                PLIC_DisableIRQ_static(MAC1_EMAC_PLIC);
