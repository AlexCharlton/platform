...
#define SETUP_PKT_INIT()          \
    ;                             \
    g_setup_pkt.request_type = 0; \
...
/*usbd_test_cb.test_suspend();*/
    ...
    */
    ...
    static void mss_usbd_disconnect_cb(void)
    {
        ...
        PLIC_EnableIRQ_static(USB_MC_PLIC);
             ...
             (g_setup_pkt.request_type & USB_STD_REQ_DATA_DIR_MASK)))
        {
                ...
                (g_setup_pkt.request_type & USB_STD_REQ_RECIPIENT_MASK))
            {
                ...
                case USB_STRING_DESCRIPTOR_TYPE:
                    ...
                    {
                        *buf_pp = g_usbd_user_descr_cb->usbd_string_descriptor((uint8_t)g_setup_pkt.value, len_p);
                        return (USB_SUCCESS);
                    }
                    else
                    {
                ...
                case USB_DEVICE_QUALIFIER_DESCRIPTOR_TYPE:
                    if (MSS_USB_DEVICE_HS == g_usbd_user_speed)
                    {
                        ...
                        {
                            *buf_pp = g_usbd_user_descr_cb->usbd_device_qual_descriptor(g_usbd_dev_conf.device_speed, len_p);
                            return (USB_SUCCESS);
                        }
                        else
                        {
                ...
                case USB_CONFIGURATION_DESCRIPTOR_TYPE:
                    ...
                    {
                        *buf_pp = g_usbd_class_cb->usbd_class_get_descriptor(USB_STD_REQ_RECIPIENT_DEVICE,
                     ...
                     (g_setup_pkt.request_type & USB_STD_REQ_RECIPIENT_MASK))
            {
                if (0 != g_usbd_class_cb->usbd_class_get_descriptor)
                {
                    *buf_pp = g_usbd_class_cb->usbd_class_get_descriptor(USB_STD_REQ_RECIPIENT_INTERFACE,
                     ...
                     (g_setup_pkt.request_type & USB_STD_REQ_RECIPIENT_MASK))
            {
                ...
                {
                    *buf_pp = g_usbd_class_cb->usbd_class_get_descriptor(USB_STD_REQ_RECIPIENT_ENDPOINT,
                                                                         (g_setup_pkt.value >> USB_WVALUE_HIBITE_SHIFT),
                                                                         len_p,
                                                                         g_usbd_dev_conf.device_speed);
                    return (USB_SUCCESS);
        ...
        if (MSS_USB_ADDRESS_STATE == g_usbd_dev_conf.device_state)
        {
                ...
                (2u == g_setup_pkt.length))
            {
                ...
                case USB_STD_REQ_RECIPIENT_ENDPOINT: /*Endpoint halt (stall) status*/
                    g_usbd_status = ((gd_tx_ep[g_setup_pkt.index].stall) ? 0x0001 : 0x0000);
        ...
        else if (MSS_USB_CONFIGURED_STATE == g_usbd_dev_conf.device_state)
        {
            if ((0u == g_setup_pkt.value) && (2u == g_setup_pkt.length))
            {
                ...
                case USB_STD_REQ_RECIPIENT_ENDPOINT:
                    ...
                    {
                        g_usbd_status = ((gd_rx_ep[g_setup_pkt.index].stall) ? 0x0001 : 0x0000);
                        *buf_pp = (uint8_t *)&g_usbd_status;
                    }
                    break;
