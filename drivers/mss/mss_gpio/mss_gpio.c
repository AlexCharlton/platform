...
#define NB_OF_GPIO_INTR ((uint32_t)41)
        ...
        mss_gpio_id_t port_id)
    {
        ...
        if (0U == gpio_number_validate(gpio, port_id))
        {
            ...
            if ((GPIO0_LO == gpio) || (GPIO0_HI == gpio))
            {
                PLIC_EnableIRQ_static(g_gpio_irqn_lut[port_id]);
            }
            else if ((GPIO1_LO == gpio) || (GPIO1_HI == gpio))
            {
                PLIC_EnableIRQ_static(g_gpio_irqn_lut[port_id +
        ...
        mss_gpio_id_t port_id)
    {
        ...
        if (0U == gpio_number_validate(gpio, port_id))
        {
            ...
            if ((GPIO0_LO == gpio) || (GPIO0_HI == gpio))
            {
                PLIC_DisableIRQ_static(g_gpio_irqn_lut[port_id]);
            }
            else if ((GPIO1_LO == gpio) || (GPIO1_HI == gpio))
            {
                PLIC_DisableIRQ_static(g_gpio_irqn_lut[port_id +
            ...
            {
                PLIC_DisableIRQ_static(GPIO2_NON_DIRECT_PLIC);
            }
            else
            {
                ASSERT(0); /*LDRA warning*/
        ...
        GPIO_TypeDef const *gpio)
    {
        if ((GPIO0_LO == gpio) || (GPIO0_HI == gpio))
        {
            PLIC_EnableIRQ_static(GPIO0_NON_DIRECT_PLIC);
        }
        else if ((GPIO1_LO == gpio) || (GPIO1_HI == gpio))
        {
            PLIC_EnableIRQ_static(GPIO1_NON_DIRECT_PLIC);
        }
        else if ((GPIO2_LO == gpio) || (GPIO2_HI == gpio))
        {
            PLIC_EnableIRQ_static(GPIO2_NON_DIRECT_PLIC);
        }
        else
        {
            ASSERT(0); /*LDRA warning*/
        ...
        GPIO_TypeDef const *gpio)
    {
        if ((GPIO0_LO == gpio) || (GPIO0_HI == gpio))
        {
            PLIC_DisableIRQ_static(GPIO0_NON_DIRECT_PLIC);
        }
        else if ((GPIO1_LO == gpio) || (GPIO1_HI == gpio))
        {
            PLIC_DisableIRQ_static(GPIO1_NON_DIRECT_PLIC);
        }
        else if ((GPIO2_LO == gpio) || (GPIO2_HI == gpio))
        {
            PLIC_DisableIRQ_static(GPIO2_NON_DIRECT_PLIC);
